      ...
      dark:hover:shadow-[0_0_15px_#00ff75,0_0_25px_#3700ff]">
        ...
        <div class="flex flex-wrap gap-2 mb-3">
            ...
            @if($post->user_id === Auth::id())
                <a href="{{ route('posts.edit', $post->id) }}" class="text-green-600 hover:underline dark:text-green-400">
                    Editar
                </a>
            @endif

            {{-- Deshabilitar si es propietario o admin --}}
            @if(Auth::check() && (($post->user_id === Auth::id()) || (Auth::user()->role === 'admin')))
                <form action="{{ route('posts.disable', $post->id) }}" method="POST" class="inline-block">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="text-red-600 hover:underline dark:text-red-400">
                        Deshabilitar
